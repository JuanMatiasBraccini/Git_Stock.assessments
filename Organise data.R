...
fn.import.catch.data=function(KTCH.UNITS)
{
  ...
  #..Historic
  Historic.ktch=fn.in(NM='recons_Hist.expnd.csv')
  ...
  Tot.ktch=rbind(subset(Data.monthly,select=c(SPECIES,FishCubeCode,FINYEAR,Region,Data.set,LIVEWT.c,BLOCKX,METHOD)),
                 subset(Data.monthly.north,select=c(SPECIES,FishCubeCode,FINYEAR,Region,Data.set,LIVEWT.c,BLOCKX,METHOD)))%>%
...
KtCh=fn.import.catch.data(KTCH.UNITS)    #this should only be run once!!

fn.extract.dat=function(STRING) grepl(STRING, nm.Dat, perl = TRUE)
fn.rename.dat=function(x,y) str_remove_all(x, paste(y, collapse = "|"))

fn.input.data.new=function(Name,SP,Species,First.year,Last.year,Min.obs,Min.shts)   #this is run for each assessed species...
{
  #----DATA SECTION------ 
  ...
  #2. All catches
  Katch=KtCh%>%filter(SPECIES%in%Species & FINYEAR%in%Used.yrs)
  ...
  nm.Dat=names(Dat)
    ...
    #3.1 RELATIVE ABUNDANCE    
      #3.1.1  TDGDLF  
  #monthly
  iid=nm.Dat[fn.extract.dat(STRING="(?=.*annual.abundance.basecase)(?=.*relative)(?=.*monthly)")]
  if(length(iid)>0)
  {
  ...
  #daily
  iid=nm.Dat[fn.extract.dat(STRING="(?=.*annual.abundance.basecase)(?=.*relative)(?=.*daily)")]
  if(length(iid)>0)
  {
  ...
  iid=nm.Dat[fn.extract.dat(STRING="(?=.*inch.raw)")]
  if(length(iid)>0)
  {
    ...
    iid=nm.Dat[fn.extract.dat(STRING="(?=.*Numb_obs_size.freq.TDGDLF)")]
  ...
  iid=nm.Dat[fn.extract.dat(STRING="(?=.*Pilbara_Trawl)")]
  ...
  iid=nm.Dat[fn.extract.dat(STRING="(?=.*NSF.LONGLINE)")]
  ...
  if(Name%in%c("Gummy shark","Whiskery shark","Dusky shark","Sandbar shark"))
  {
    library(RODBC)
    library(lubridate)
  ...
  if(Name=='Gummy shark')Gummies.prop=read.csv("C:/Matias/Analyses/Catch and effort/Gummies.prop.csv",stringsAsFactors=F)
  ...
  historic=Historic.ktch%>%filter(SPECIES%in%Species)
 ...
 dev.off()
  ...
  #6.Export data for use in models    #REVIEW THIS, seems un necessary....
...
fn.input.data.new(Name='Sandbar shark',SP="TK",Species=18007,
                  First.year="1975-76",Last.year="2017-18",
                  Min.obs=10,Min.shts=10)  #move this to Run.model.R
