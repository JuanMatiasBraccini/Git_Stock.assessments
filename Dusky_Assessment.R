# ------ Script for running dusky shark stock assessment ---- ###################
...
Max.FL.obs=175      #change accordingly    MISSING

  #Size at birth
Lo=33               #change accordingly    MISSING
...
if(do.Level5=="YES")
{
  ...
  M_val=0.283          #change accordingly    MISSING
  M_val.low=0.22      #using Hoenig set to Max age=19 exp(1.46-1.01*log(19))  #change accordingly    MISSING
  M_val.high=0.32     #using Hoenig set to Max age=13 exp(1.46-1.01*log(13))  #change accordingly    MISSING
  ...
  Tabla.scen=data.frame(                      #change accordingly    MISSING
    Model=Models,
  ...
  #Define groups for comparing model outputs            #change accordingly    MISSING
  ...
  #Estimable parameter phases       #change accordingly  MISSING
  Fz.off=-22
  Par.phases=Pin.pars
...
ct.NSF=read.csv(paste("C:/Matias/Data/Population dynamics/Data inputs for models",Spec,AssessYr,"ktch.annual.NSF.csv",sep="/"))
ct.Other=read.csv(paste("C:/Matias/Data/Population dynamics/Data inputs for models",Spec,AssessYr,"ktch.annual.Other.csv",sep="/"))
ct.Rec=read.csv(paste("C:/Matias/Data/Population dynamics/Data inputs for models",Spec,AssessYr,"ktch.annual.Rec.csv",sep="/"))
ct.TDGDLF=read.csv(paste("C:/Matias/Data/Population dynamics/Data inputs for models",Spec,AssessYr,"ktch.annual.TDGDLF.csv",sep="/"))
ct.TDGDLF$LIVEWT.c=ct.TDGDLF$LIVEWT.c/1000
ct.TEPS=read.csv(paste("C:/Matias/Data/Population dynamics/Data inputs for models",Spec,AssessYr,"ktch.annual.TEPS.csv",sep="/"))

names(ct.NSF)[match("LIVEWT.c",names(ct.NSF))]="LIVEWT.c.NSF"
names(ct.Other)[match("LIVEWT.c",names(ct.Other))]="LIVEWT.c.Other"
names(ct.Rec)[match("LIVEWT.c",names(ct.Rec))]="LIVEWT.c.Rec"
names(ct.TDGDLF)[match("LIVEWT.c",names(ct.TDGDLF))]="LIVEWT.c.TDGDLF"
names(ct.TEPS)[match("LIVEWT.c",names(ct.TEPS))]="LIVEWT.c.TEPS"

ct=merge(ct.NSF,ct.Other,by="FINYEAR",all=T)
ct=merge(ct,ct.Rec,by="FINYEAR",all=T)
ct=merge(ct,ct.TDGDLF,by="FINYEAR",all=T)
ct=merge(ct,ct.TEPS,by="FINYEAR",all=T)
ct[is.na(ct)]=0
ct$Total.ktch=with(ct,LIVEWT.c.NSF+LIVEWT.c.Other+LIVEWT.c.Rec+LIVEWT.c.TDGDLF+LIVEWT.c.TEPS)
ct$FINYEAR=as.character(ct$FINYEAR)
ct$finyear=as.numeric(substr(ct$FINYEAR,1,4))
ct=ct[order(ct$finyear),]
ct=subset(ct,finyear>1974)

#forward projections        
Current=ct$finyear[nrow(ct)]    
years.futures=5
yr.future=Current+(1:years.futures)
ct.future=rep(mean(ct$Total.ktch[(nrow(ct)-4):nrow(ct)]),years.futures)  #average of last 5 years
...
ktch_msy_scen$'Base case'=list(
  ...
  user="Yes",
  k.max=50,
  startbio=c(0.7,.95),      #lower bound a bit smaller see 'Catch_Historic & time series.jpeg) 
  finalbio=c(0.2, 0.6),
  res="Very low",
  niter=SIMS,
  sigR=Proc.err
...
#Execute Catch-MSY  #takes 0.0003 seconds per iteration per scenario
system.time({fn.source("Apply.catch_MSY.R")})
...
if(do.Level5=="YES") source("C:/Matias/Analyses/SOURCE_SCRIPTS/Population dynamics/Run.models.R")
