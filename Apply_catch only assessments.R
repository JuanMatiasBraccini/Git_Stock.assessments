...
for(w in 1:length(Catch_only))
{
  ...
  if(names(Catch_only)[w]=="DBSRA")
  {
    ...
    for(i in 1:length(dummy.store))  
    {
      ...
      Out.ensemble=list(Depletion=do.call(rbind,fn.get.stuff.from.list(Out.ensemble,'Depletion')),
      ...
      dummy.store.ensemble[[i]]=Out.ensemble
    }
    ...
    for(i in 1:length(dummy.store))  
    {
      ...
      for(s in 1:length(Store.sens))
      {
        ...
        }else
        {
          ...
          if(names(dummy.store)[i]=="milk shark") r.range[1]=0.1   #need to drop lower bound a bit to allow enough combos
  ...
  # summary of method: https://github.com/shcaba/SSS    #2.2 secs per simulation
  if(names(Catch_only)[w]=="SSS")
  {
    ...
    for(i in 1:N.sp)
    {
      ...
      for(s in 1:length(Store.sens))
      {
        ...
        if(length(Report)>0)
        {
          ...
          Out.ensemble[[s]]=list(Depletion=Depletion,
                                 B.Bmsy=B.Bmsy,
                                 F.Fmsy=F.Fmsy) 
          rm(Depletion,B.Bmsy,F.Fmsy,dum)
      ...
      print(p)
      ggsave(paste(handl_OneDrive("Analyses/Population dynamics/1."),
                   capitalize(List.sp[[i]]$Name),"/",AssessYr,"/SS3/Rel.biomass&estimates.tiff",sep=''))
    ...
    rm(dummy.store,dummy.store.sens.table,dummy.store.estimates,
       dummy.store.rel.biom,dummy.store.probs.rel.biom,dummy.store.f.series,
       dummy.store.B.Bmsy,dummy.store.F.Fmsy,dummy.store.ensemble,
       dummy.store.warnings,dummy.store.convergence)
...
if(do.ensemble.simulations)   #56 sec per iteration under do.full.sims=FALSE (iterations= r.groups(n=3) * HarvestRates (n=4) * n.reps (20))   
{
  ...
  system.time({for(s in 1:length(Inputs))   
  {
    ...
    for(n in 1:n.sims_SS)  
    {
      if(do.random.h)
      {
        h.mean=Out.Scens$Steepness[s]
        h.sim[n]=rtruncnorm(1,a=Min.h.shark, b=Max.h.shark,
                            mean=h.mean,
                            sd=max(Out.Scens$Steepness.sd[s],0.01))
        m.mean=List.sp[[i]]$Sens.test$DBSRA$Mmean[s]
        m.sim[n]=rtruncnorm(1,a=0.02, b=5,
                            mean=m.mean,
                            sd=List.sp[[i]]$Sens.test$DBSRA$Msd[s])
      }else
      {
        h.sim[n]=LH.sim$h[n]
        if(Keep.species[i]=="grey nurse shark")  #too low h for greynurse
...
#18.2.2. sample each accepted COM proportionally to weights  
...
#19.2 Table of parameter estimates by species and catch-only assessment method
for(l in 1: length(Lista.sp.outputs))
{
  for(w in 1:length(Catch_only))
  {
...
#19.3.1 Display all scenarios for each species   
YLAB=XLAB=''
#19.3.1.1 Relative biomass (i.e. Depletion)
...
#19.3.2 Display Scenario 1 for combined species: each COM model as an Appendix 
...
#19.3.3 Display Scenario 1 for combined species: COM model-average  
...
#19.4.2 Display combined species: each COM model as an Appendix
do.this=FALSE
if(do.this)
{
  for(l in 1:length(Lista.sp.outputs))
...
#19.4.3 Display combined species: COM model-average 
...
#19.5 Percentage acceptance rate for Scenario 1 (not applicable to JABBA or SSS)   
...
#19.6 MSY estimates by Lista.sp.outputs (Scenario 1)   #redundant, MSY output in #20.2
# for(l in 1: length(Lista.sp.outputs))
# {
