  ...
  if(Scenario$Model=='SS') 
  {
    ...
    if(!is.null(size.comp) | !is.null(meanbodywt))
    {
      ...
      if(length(id.fleet.to.estim)>0)
      {
        ...
        id.fleet.to.estim=grep(id.fleet.to.estim,rownames(ctl$size_selex_parms))
        if(length(id.fleet.to.estim)>0) ctl$size_selex_parms[id.fleet.to.estim,'PHASE']=3
      }
    }
  ...
  if(do.like.prof) # 30 secs per species-parameter-iteration if in parallel and with hessian; 
  {
    ...
    for(pp in 1:length(Par_var_profile))
    {
      ...
      SSplotProfile1(summaryoutput=prof.summary,
                     ...
                     profile.label=PR_la,
                     Xvec=VEC.prof,
                     minfraction = 0.0001,  #show like comp that change Total like at least by this much
                     pheight=4.5,
                     print=FALSE,
                     plotdir=plotdir,
...
see.SS3.length.comp.matrix=function(dd,LVLS,yr.LVLS)
{
  dd=dd%>%
    rowwise() %>%
  ...
  pp=dd%>%
    ...
    mutate(Fleet=as.character(Fleet),
           ...
           Fleet=factor(Fleet,levels=LVLS),
           year=factor(year,levels=yr.LVLS))
  ...
  d.lbl=pp%>%
    ...
    mutate(Nsamp=paste0(Nsamp,' shots'),
           Fleet=factor(Fleet,levels=LVLS),
           year=factor(year,levels=yr.LVLS))%>%
    distinct(year,Fleet,Nsamp,.keep_all = TRUE)
  
  pp=pp%>%
      ggplot(aes(Size,n))+
      geom_line(aes(color=Fleet, group = Fleet))+
