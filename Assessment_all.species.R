...
#Description: 
#     The script first defines arguments used in each of the shark species/species complex assessed.
#     It then run the relevant population models according to data availability
#     Finally, the script generates relevant outputs
#     A range of assessment methods are used depending on data availability: 
#               . Integrated size- and sex- structured models,
#               . SPM,
#               . Catch-MSY (Martell & Froese 2012)
#               . aSPM(Haddon SimpleSA())
#     Total catches are used (extracted from commercial and recreational catch reconstructions)
#     Assumption: If catches have never been >1% carrying capacity, then it's in unexploited status so catch series have
#      no information on productivity. A PSA analysis is used initially to filter out species



#missing:
# Update input parameters for Dusky and Sandbar
...
#---Data section-----   
Dat.repository='C:/Matias/Analyses/Data_outs/'  #where all data are located
...
assessed.elsewhere=c("white shark","school shark","spot-tail shark","blacktips","unidentified sharks")

  #species list for exploring spatial dist of catch     #REMOVE ALL this if not used!!!
...
non.sharks=c("Australian Salmon","Baldchin groper",                          
...
KtCh.method=Get.ktch$Total.method
...
KtCh.method=KtCh.method%>%filter(!Name%in%assessed.elsewhere)
...
{
  hNdl='C:/Matias/Analyses/Population dynamics/Other species paper/2020'
...
for(l in 1:N.sp)
{
  ...
  if(!List.sp[[l]]$Species%in%Indicator.species)
  {
    List.sp[[l]]=list.append(List.sp[[l]],
      ...
      #... Catch-MSY arguments
      SIMS=5e4,   #simulatins
      ERROR=0,   #Assumed process error
        #depletion level at start of catch series
      STARTBIO=c(List.sp[[l]]$B.init*.95,List.sp[[l]]$B.init),   #low depletion because starting time series prior to any fishing
      FINALBIO=c(.2,.9),       #very uncertain
      Do.sim.test="NO",  #simulation test Catch-MSY for small and large catches
    ...
    List.sp[[l]]$SCENARIOS=SCENARIOS
    List.sp[[l]]$SCENARIOS$BaseCase=list(Error=List.sp[[l]]$ERROR,
                                         R.prior=List.sp[[l]]$r.prior,
                                         Initial.dep=List.sp[[l]]$STARTBIO)
    if('WorstCase'%in%names(List.sp[[l]]$SCENARIOS$WorstCase))
    {
      List.sp[[l]]$SCENARIOS$WorstCase=list(Error=List.sp[[l]]$ERROR,
                                            R.prior=List.sp[[l]]$r.prior2,
                                            Initial.dep=List.sp[[l]]$STARTBIO)
    }
...
# 6. Execute population models From indicator species Assessment.R code-------------------------------
