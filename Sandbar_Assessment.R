# ------ Script for running sandbar shark stock assessment ---- ###################
...
ct[is.na(ct)]=0
ct$Total.ktch=with(ct,LIVEWT.c.NSF+LIVEWT.c.Other+LIVEWT.c.Rec+LIVEWT.c.TDGDLF)
ct$FINYEAR=as.character(ct$FINYEAR)
ct$finyear=as.numeric(substr(ct$FINYEAR,1,4))
ct=ct[order(ct$finyear),]
ct=subset(ct,finyear>1977)
...
ktch_msy_scen$'Base case'=list(
  ...
  user="Yes",
  k.max=50,
  startbio=c(0.85,.95),       #lower bound a higher smaller see 'Catch_Historic & time series.jpeg)
  finalbio=c(0.2, 0.6),
  res="Very low",
  niter=SIMS,
  sigR=Proc.err   
...
#Execute Catch-MSY  #takes 0.0001 seconds per iteration per scenario
